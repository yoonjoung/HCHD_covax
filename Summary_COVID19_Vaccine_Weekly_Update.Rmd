...
Data source: *Immunet* from Maryland Department of Health, as of `r date`. For further data analysis, methods, and detailed note on data sources, [please see the full note here](https://rpubs.com/YJ_Choi/HCHD_covax).     
...
dtaimmunetNEW<-read_excel("~/Dropbox/0 iSquared/iSquared_Harfod/dta_immunet/COVID-19 Administrations - Harford 12032021.xlsx")
...
dtaeligible<-dta%>%
    ...
    mutate(
        ...
        eligible=
            (complete==1 & firstvaccine==1 & dosenumber==2 & intervalsince>=180 & age>=18) | 
            (complete==1 & firstvaccine==2 & dosenumber==2 & intervalsince>=180) | 
...
dtabooster<-dta%>%
    ...
    mutate(    
        #temporary var needed to create a person-level eligibility 
        #Age limit changed to 18 on Nov 29
        eligibleforbooster=
            (complete==1 & firstvaccine==1 & dosenumber==2 & intervalsince>=180 & age>=18) | 
            (complete==1 & firstvaccine==2 & dosenumber==2 & intervalsince>=180 & age>=18) | 
            (complete==1 & firstvaccine==3 & dosenumber==1 & intervalsince>=60) 
    )%>%
    ...
    mutate(
        ...
        booster_eligiblePFR = firstvaccine==1 & dosenumber>=3 & interval>=180 & age>=18, 
        #booster_eligibleMOD = firstvaccine==2 & dosenumber>=3 & interval>=180 & age>=18, 
        booster_eligibleMOD = firstvaccine==2 & dosenumber>=3 & interval>=180,
        ...
        booster_ineligiblePFR = firstvaccine==1 & dosenumber>=3 & interval>=180 & age<18, 
        #booster_ineligibleMOD = firstvaccine==2 & dosenumber>=3 & interval>=180 & age<18, 
...
pctdecrease<- round(100*(nboosterlastweek - nboosterpreviousweek) /nboosterpreviousweek, 1)
...
dtaboosterdaily<-dtabooster%>%
    ...
    mutate(
        ...
        place="3.Other: Neither pharmacy nor hospital",
        place=ifelse(pharmacy==1, "1.Pharmacy", 
                     ifelse(hospital==1, "2.Hospital",
                            place))
    )%>%
...
1. __In the past 7 days, `r nboosterlastweek` booster shots were provided: a change by `r pctdecrease`%__, compared to `r nboosterpreviousweek` in the previous 7-days. A total of `r nbooster` "booster" shots were given to date.       
2. __In the last four weeks, the number of booster shots for 60+ has declined continuously__, while the number for <60 has remained stable.    
3. Pharmacies are main source for booster shots. In the past 14 days, `r ntotal` booster shots were provided: `r pct_pharmacy`% of them were given at pharmacies and `r pct_hospital`% were given at hospitals.     
</div>
...
dtafig%>%
    ...
    plot_ly(
        x=~retroweekdate,
        y=~booster_eligiblePFR, name="18+ & PFR base shot", 
        type = 'bar',
        marker=list(color = c("#1F77B4"), size=5))%>%
    add_bars(
        y=~booster_eligibleMOD, name="MOD base shot",
        marker=list(color = c("#6AA84F"), size=5))%>%
    add_bars(
        y=~booster_eligibleJSN, name="JAN base shot",
        marker=list(color = c("#D98DB9"), size=5))%>%
    add_bars(
        y=~booster_ineligiblePFR, name="<18 & PFR base shot",
        marker=list(color = c("#A4C7E1"), size=5))%>%
    #add_bars(
    #    y=~booster_ineligibleMOD, name="<18 & MOD base shot",
...
See [Annex 2 in the full note](https://rpubs.com/YJ_Choi/HCHD_covax) for how booster shots are defined.     
</div>
...
1. Simply based on the age, type and date of the initial shot, a total of `r nbooster_eligible` shots were given to people "who should get booster shots", according to [CDC's guidelines](https://www.cdc.gov/coronavirus/2019-ncov/vaccines/booster-shot.html). **This is `r pctboostereligible` % of the total `r nboostereligible` people who should get booster to date**.     
2. Booster rates vary by age, race, and brand/type of the first dose. [Further breakdown by _both manufacturer and race_](https://rpubs.com/YJ_Choi/HCHD_covax) shows that booster rates are lower among Hispanic and Black eligible groups compared to other groups, even when they have received the same brand/type of the first dose.              
</div>
...
temppct<-rbind(temp1pct, temp2pct, temp4pct)
tempobs<-rbind(temp1obs, temp2obs, temp4obs)
...
names(dtafig)<-c("Group", 
                     " ",
                     "Number of eligible* (*should get booster)", 
                     "Percent of eligible peopole who received booster"
                     ) 
...
kable(dtafig, caption = "Percent of eligible* people who received booster: by background")%>%
    ...
    column_spec(2, width = "10em", background = "#EEEEEE")%>%
    column_spec(3, width = "10em")%>%
    column_spec(4, width = "10em", background = "#EEEEEE")
...
dtaweeklytrendage$agegroup <- factor(dtaweeklytrendage$agegroup, 
                                  levels = c("5-11", "12-15", "16-19", "20-39"))

```

```{r}
temp<-dtaweeklytrendage%>%
    filter(agegroup == "5-11")%>%
    filter(retroweekdate == max(retroweekdate))
    
pct_cum_first_511<-round(temp$pct_cum_first, 2)
pct_cum_first_511
num_first_511<-temp$obsweeklyfirst
num_first_511
cum_first_511<-temp$cum_first
cum_first_511

temp<-dtaweeklytrendage%>%
    filter(agegroup == "12-15")%>%
    filter(retroweekdate == max(retroweekdate))

pct_cum_first_1215<-round(temp$pct_cum_first, 0)
pct_cum_first_1215
```

```{r trend_weekly_people_where_type12}
...
```{r}
temp<-dtafirstweeklywhere%>%
    filter(retroweekdate == max(dtafirstweeklywhere$retroweekdate))%>%
    mutate(
        pct=round(100*obs/sum(obs), 0),
        groupname="Other", 
        groupname=
            ifelse(group==0, "HCHD",
                ifelse(group==1, "Massvax sites",
                    ifelse(group==2, "Mobilevax sites",
                        ifelse(group==3, "Pharmacies", 
                            ifelse(group==4, "Pediatric Partner",
                                ifelse(group==5, "Other pediatric practices",
                                    group))))))
        )

temptemp<-temp%>%filter(pct==max(pct))
    
commonsource<-print(temptemp$groupname)
commonsource        
pctcommonsource<-temptemp$pct
pctcommonsource

temptemp<-temp%>%filter(group==4 | group==5)
pctpeds<-sum(temptemp$pct)
pctpeds
```
...
<strong>Among 12-15:</strong>     
1. __It has remained lowest across all age groups in 12 or older__ (`r pct_cum_first_1215`% of the population received one or more doses).     
2. __Pharmacies__ have been the main source of vaccination, partially because massvax sites were closed soon after they became eligible.    
...
<strong>Among 5-11:</strong>      
1. In the last 7 days, `r num_first_511` children received their first dose. To date, a total of `r cum_first_511` first dose vaccinations have been provided to children 5-11 years of age.     
2. To date, `r pct_cum_first_511`% of the population received one or more doses.     
2. In the last 7 days, the most common source was `r commonsource`, vaccinating `r pctcommonsource`% of the young children. `r pctpeds`% were vaccinated at pediatric practices. 
</div>
...
dtafirstweeklytrend_age<-dta%>%
    ...
    mutate(
        ...
        agegroup=ifelse(age<=11, "0-9", agegroup),
        agegroup=ifelse(agegroup=="0-9", "5-11", agegroup),
        obs=1
        )%>%
    #filter(agefloor>=10)%>% as of 11/2/2021
    group_by(agegroup, retroweekdate)%>%
...
dtafig$agegroup <- factor(dtafig$agegroup, 
                          levels = c("5-11", "12-19", "20-29", "30-39", "40-49", 
                                     "50-59", "60-69", "70-79", "80+"))

dim(dtafirstweeklytrend_age)
dim(dtapop)
dim(dtafig)
table(dtafig$agegroup)
```

```{r plot_trend_weekly_age}
dtafig%>%
    plot_ly(x = ~retroweekdate,
...
dtafig<-dta%>%
    ...
    select(retroweekdate, agegroup, obs)%>%
...
dtafig%>%
    ...
    layout(
        title = c("Weekly number of first shots by age-group"),
...
### 3.2. Among those __under 16 years__, where do they get vaccinated?    
```{r trend_weekly_people_where_type16}
dtafirstweeklywhere<-dta%>%
    filter(age<16)%>%
    filter(dosenumber==1)%>%
    filter(is.na(vdate)==FALSE)%>%
...
```{r plot_trend_weekly_people_where_type16, results="asis"}
dtafig%>%
    plot_ly(x=~retroweekdate,
        y=~HCHD, name="HCHD",
        type = 'bar',
    ...
    layout(
        barmode = 'stack',
        title = c("Weekly number of first dose vaccination by type of place"),
        xaxis=list(title = "Date, last in the week",  
                 font=list(size=8),
                 tickfont = list(size=8),
                 showgrid = FALSE
...
```{r trend_byStatus_weekly_people_age_same}
dtafirstweekly<-dta%>%
    filter(dosenumber==1)%>%
    filter(is.na(vdate)==FALSE)%>%
    #filter(age<40)%>%
...
```{r plot_trend_byStatus_weekly_people_age, results="asis", out.width="1000px"}
panel <- . %>%
    plot_ly(x=~retroweekdate,
