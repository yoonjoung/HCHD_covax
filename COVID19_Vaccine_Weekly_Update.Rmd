...
Data from Maryland Department of Health, as of `r date`. See Annex for detailed note about the data sources and methods.    

```{r directory_check}
dir("~/Dropbox/0 iSquared/iSquared_Harfod/dta_immunet")
```

```{r dta_import}
# CAUTION ################################################    
# Starting from May 3, the data has only vaccination on March 1 or later!!!
# Also, last data from Cherry/MDH is on December 3 (i.e., data through Dec 2)
# THEREFORE, use three data files  
#   1. updated data from Ronya - which has data from November 1
#   2. COVID-19 Administrations - Harford 12032021.xlsx 
#       (latest data from Cheery)
#   3. COVID-19 Administrations - Harford 04302021.xlsx 
#       (latest data from Cherry that has data prior to March 1)

#Immunization level data - bring data from Ronya  
#dtaimmunetNEW <- read_csv("~/Dropbox/0 iSquared/iSquared_Harfod/dta_immunet/Harford 12.8.21.csv")
dtaimmunetNEW <- read_excel("~/Dropbox/0 iSquared/iSquared_Harfod/dta_immunet/Harford 12.8.21.xlsx")
#str(dtaimmunetNEW)

dtaimmunetNEW<-dtaimmunetNEW%>%
    rename(
        VACCINE_TYPE	 = 	'Vaccine group',
        ADMINISTERING_ORG_NAME	 = 	'NPI'	, 
        #CLIENT_ID	 = 		, 
        ...
        STREET_ADDRESS_LINE	 = 	'Street'	, 
        OTHER_ADDRESS_LINE	 = 	'Other address'	, 
        CITY_NAME	 = 	'City'	, 
        STATE_CODE	 = 	'State'	, 
        ZIP_CODE	 = 	'Zipcode'	, 
...
#Immunization level data - bring data from March 1 - November 30  
dtaimmunetOLD2<-read_excel("~/Dropbox/0 iSquared/iSquared_Harfod/dta_immunet/COVID-19 Administrations - Harford 12032021.xlsx")%>%
    mutate(vdate = as.Date(VACCINATION_DATE, "%d-%b-%Y"))%>%
    filter(vdate >= as.Date("2021-03-01") & vdate < as.Date("2021-12-01"))

#Immunization level data - bring data prior to March 1
dtaimmunetOLD1<-read_excel("~/Dropbox/0 iSquared/iSquared_Harfod/dta_immunet/COVID-19 Administrations - Harford 04302021.xlsx")%>%
    mutate(vdate = as.Date(VACCINATION_DATE, "%d-%b-%Y"))%>%
...
str(dtaimmunetNEW$vdate)
str(dtaimmunetOLD2$vdate)
```
colnames(dtaimmunetNEW)
colnames(dtaimmunetOLD2)
str(dtaimmunetNEW)
str(dtaimmunetOLD2)

dtaimmunet<-rbind(dtaimmunetOLD2, dtaimmunetOLD1)%>%
    select(-vdate)
...
Clinic site,
Dose Number,
Funding Type VFC Elig Override,
Historical Org Name/POD ID,
Number of Doses in Series,
VFC Eligibility Description,
...
NOTE: "Weekly data" in this note is not data for a calendar week. It refers to a seven-day period leading up to the date of latest update (e.g., last Thursday-Wednesday, if an update is on Thursday).   
</div>
## 0. Coverage: booster
...
#"For individuals who received a Pfizer-BioNTech or Moderna COVID-19 vaccine, the following groups __"should"__ get a booster shot at 6 months or more after their initial series:
#- __50 years and older__   
#- Age 18+ who live in long-term care settings   
#- Age 18+ who have underlying medical conditions   
#- Age 18+ who work or live in high-risk settings"   
#Also, 
#"People age 18 years and older who received Pfizer-BioNTech or Moderna COVID-19 vaccines __may__ get a booster."

#For the purpose of this report, eligible people refer to __people who should get booster__, according to the guidelines, although 18+ are clinically eligible.     
```

As of November 29, 2021, according to **[CDC's guidelines](https://www.cdc.gov/coronavirus/2019-ncov/vaccines/booster-shot.html)**:         
"Everyone ages 18 and older should get a booster shot" - 6 (PFR/MOD) or 2 (JSN) months after  completing their primary COVID-19 vaccination.
...
dtaeligibleweekly<-full_join(datesequence, dtaeligible, by="vdate")%>%
    ...
    mutate(
        group="",
        #group=ifelse(firstvaccine==1 & age>=50, "50+ PFR",
        #      ifelse(firstvaccine==1 & age<50, "<50 PFR",
        #      ifelse(firstvaccine==2 & age>=50, "50+ MOD",
        #      ifelse(firstvaccine==2 & age<50, "<50 MOD",         
        #      ifelse(firstvaccine==3 , "JSN",         
        #             group))))), 
        
        group=ifelse(firstvaccine==1 & age>=50, "50+ PFR",
              ifelse(firstvaccine==1 & age<50 & age>=18, "18-49 PFR",
              ifelse(firstvaccine==1 & age<18, "<18 PFR**",         
              ifelse(firstvaccine==2 & age>=50, "50+ MOD",
              ifelse(firstvaccine==2 & age<50, "18-49 MOD",         
              ifelse(firstvaccine==3 , "JSN",         
                     group)))))),          
...
dtaeligibleweekly$group <- factor(dtaeligibleweekly$group, 
                                  levels = c("50+ PFR", "50+ MOD", "JSN", "18-49 PFR", "18-49 MOD", "<18 PFR**"))
...
```{r}
table(dtabooster$booster_ineligiblePFR, dtabooster$MANUFACTURER_first) 
```
...
### 1.2A.1. Among those younger than 16, in what type of facilities have they been vaccinated __for their first shot__?  
...
```{r plot_trend_weekly_people_where_type18}
dtafig%>%
    plot_ly(x=~retroweekdate,
        y=~HCHD, name="HCHD",
        type = 'bar',
...
- Single year population for 0-19 years of age came from the county government.       
...
- Age is __age at the first vaccination__, to avoid rolling changes in denominators (REVISION on 11/2/2021).      
