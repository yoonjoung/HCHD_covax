...
dtabreak<-dtabreak%>%
    ...
    mutate(
        ...
        Variant.Lineage = replace(Variant.Lineage, 
                                  ...
                                  grepl("Q", Variant.Lineage)==TRUE |
                                  grepl("Alpha", Variant.Lineage)==TRUE, 
                                  "VBM: Alpha"),
        Variant.Lineage = replace(Variant.Lineage, 
                                  grepl("B.1.351", Variant.Lineage)==TRUE |
                                  grepl("Beta", Variant.Lineage)==TRUE, 
                                  "VBM: Beta"),
        Variant.Lineage = replace(Variant.Lineage, 
                                  grepl("P.1", Variant.Lineage)==TRUE |
                                  grepl("Gamma", Variant.Lineage)==TRUE, 
                                  "VBM: Gamma"),
        ...
        Variant.Lineage = replace(Variant.Lineage, 
                                  ...
                                  grepl("Epsilon", Variant.Lineage)==TRUE, 
                                  "VBM: Epsilon"),        
        Variant.Lineage = replace(Variant.Lineage, 
        ...
        Variant.Lineage = replace(Variant.Lineage, 
                                  ...
                                  grepl("Zeta", Variant.Lineage)==TRUE, 
                                  "VBM: Zeta"), 
        Variant.Lineage = replace(Variant.Lineage, 
                                  grepl("B.1.621", Variant.Lineage)==TRUE|
                                  grepl("B.1.621.1", Variant.Lineage)==TRUE|
                                  grepl("Mu", Variant.Lineage)==TRUE, 
                                  "VBM: Mu")
)
...
dtabreak<-dtabreak%>%
    mutate(
        Variant.Lineage = replace(Variant.Lineage, 
                                  Lineage.Type!="" & 
                                  grepl("VBM:", Variant.Lineage)==FALSE & 
                                  grepl("VOC:", Variant.Lineage)==FALSE & 
                                  grepl("VOI:", Variant.Lineage)==FALSE & 
                                  grepl("VOHC:", Variant.Lineage)==FALSE, 
                                  "Wild Type")
    )
table(dtabreak$Variant.Lineage, dtabreak$Lineage.Type)
table(dtabreak$Variant.Lineage)
...
#                      levels = c("11 and Under", "12-19","20-29","30-39","40-49","50-59","60-69","70+"))
...
The list of VBM, VOI, VOC, and VOHC updated as of __November 30, 2021__.  
https://www.cdc.gov/coronavirus/2019-ncov/variants/variant-info.html

__Variants Being Monitored (VBM)__:     
...
__Variants of interest (VOI)__:      
Currently none.   

__Variants of Concern (VOC)__:   
* Delta: B.1.617.2 and AY lineages

__Variant of High Consequence (VOHC)__: 
Currently none.   
</div>
...
### 4. Among Harford county residents who were hospitalized, vaccination status at the time of specimen collection - weekly trends since March 2021 and monthly trends since the beginning of the pandemic      
...
dtahospmonthly<-dtahospdaily%>%
    group_by(retromonthdate, vaxstatus)%>%
    summarize_at(vars(obs), funs(sum(., na.rm = TRUE)))%>%
    ungroup()
...
table(dtahosp$vaxstatus)
table(dtahospdaily$vaxstatus)
table(dtahospmonthly$vaxstatus)
    
```

```{r plot_trend_monthly_vaxstatus_hosp, results="asis", out.width="1000px"}
...
dtafig%>% 
    ...
    layout(
        title = c("Monthly distribution of hospitalization cases by vaccination status at the time of testing*: Harford"),
        yaxis = list(title = "Number of hospitalization cases",
                     titlefont=list(size=12)), 
        xaxis = list(title = "Data of specimen collection",
...
```{r plot_trend_monthly_vaxstatus_hosp_pct, results="asis", out.width="1000px"}
dtafig%>% 
...
dtahospmonthly<-dtahospdaily%>%
    group_by(retromonthdate, agevaxstatus)%>%
    summarize_at(vars(obs), funs(sum(., na.rm = TRUE)))%>%
    ungroup()
...
```{r plot_trend_monthly_agevaxstatus_hosp, results="asis", out.width="1000px"}
dtafig<-dtahospmonthly
...
```{r plot_trend_monthly_agevaxstatus_hosp_pct, results="asis", out.width="1000px"}
dtafig%>% 
    group_by(retromonthdate)%>%
    mutate(total=sum(obs))%>%
    ungroup()%>%
...
#### 4.3. Broken down only by detailed age group
    ...
    plot_ly(x = ~retroweekdate,
            y = ~pct, type = "bar",
             ...
             colors = brewer.pal(length(unique(dtafig$agegroup)),
                                "Spectral")            ) %>% 
    layout(
        title = c("Weekly distribution of hospitalization cases by age: Harford"),
        yaxis = list(title = "percent of hospitalization cases",
                     titlefont=list(size=12)), 
        xaxis = list(title = "Data of specimen collection",
                     range=c(min(dtafig$retroweekdate)-7, max(dtafig$retroweekdate)+7)),
...
```{r plot_trend_monthly_agegroup_hosp, results="asis", out.width="1000px"}
dtafig<-dtahospmonthly
...
```{r plot_trend_monthly_agegroup_hosp_pct, results="asis", out.width="1000px"}
dtafig%>% 
    group_by(retromonthdate)%>%
    mutate(total=sum(obs))%>%
    ungroup()%>%
